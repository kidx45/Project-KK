...
func TestPasetoMaker(t *testing.T) {
	maker, err := token.NewPasetoMaker(utils.RandomLetterString(32))
...
func TestMinSecretKeySizePaseto(t *testing.T) {
	maker, err := token.NewPasetoMaker(utils.RandomLetterString(31))
	require.Error(t,err)
	require.Nil(t,maker)
}
