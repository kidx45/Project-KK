...
- **Detached Mode (`-d`)**: Runs the container in the background, allowing you to use your terminal while it runs.
  - *Example:* `docker run --name some-name -p 8080:8080 -e ENV_VAR=SOME_VALUE -d image-name`
...
When testing an integration with a database, it is best to use a test database to check whether or not the genrate wriiten by the programmer doesn't have any logical and syntax error. In golang **`Testing`** package we have,
...
- **Comments**: Must follow an exact format: `-- name: <method_name> :<return_type>`. 
  - *Example:* `-- name: GetAccount :one`
  - *Note:* `--name` (without space) will not work.
